...
export default function TemplateManagerPanel({
  ...
  const formatPreviewBody = (text) => {
    ...
    let formatted = text
      ...
      .replace(
        /\{\{(\d+)\}\}/g,
        `<span class="bg-emerald-100 text-emerald-800 px-1 rounded font-mono text-[11px] mx-0.5">{{$1}}</span>`,
      );
  ...
  const filterCounts = useMemo(() => {
    const counts = {
      ALL: normalizedTemplates.length,
      APPROVED: 0,
      RECEIVED: 0,
      PENDING: 0,
      REJECTED: 0,
    ...
    if (searchQuery.trim() !== "") {
      ...
      filtered = filtered.filter(
        (t) =>
          ...
          (t.whatsapp?.category &&
            t.whatsapp.category.toLowerCase().includes(query)) ||
        ...
        <div className="flex flex-col gap-1 w-full">
          ...
          {rejectionReason && (
            ...
            >
              ...
              <p
                className={`text-[11px] text-rose-600 font-medium leading-snug whitespace-pre-wrap ${!isExpanded ? "line-clamp-2" : ""}`}
              >
                {rejectionReason}
              </p>
    ...
    if (status === "received") {
      return (
        <span className="inline-flex items-center gap-1 w-max text-[10px] font-bold uppercase tracking-widest text-green-700 bg-green-50 px-2 py-1 rounded border border-green-200">
          <Signal size={12} strokeWidth={2.5} className="text-green-500" />{" "}
          Received
        </span>
      );
    }
  ...
  return (
    <>
      ...
      {/* Drawer Panel */}
      <div
        className={`fixed top-0 right-0 h-[100dvh] w-full md:w-[600px] bg-slate-50 border-l border-slate-200 shadow-2xl z-[1000] flex flex-col transform transition-transform duration-300 ease-out ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* Header */}
        <div className="p-5 border-b border-slate-200 bg-white flex flex-col gap-4 shrink-0 z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-extrabold text-slate-800 text-lg leading-none tracking-tight">
                  Select Template
                </h3>
            ...
            <button
              className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all"
              onClick={onClose}
            >
              <X size={18} />
            </button>
            ...
            {[
              ...
              return (
                ...
                >
                  {filter}
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded-md ${activeFilter === filter ? "bg-slate-600 text-slate-200" : "bg-slate-100 text-slate-500"}`}
                  >
                    {count}
                  </span>
                </button>
          ...
          {loading && normalizedTemplates.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4 pb-4">
              {filteredTemplates.map((tpl) => {
                ...
                const isApproved =
                  tpl.normalizedStatus === "approved" ||
                  tpl.normalizedStatus === "received" ||
                  tpl.normalizedStatus === "pending";
                  ...
                  <div
                    ...
                    onClick={() =>
                      isApproved
                        ...
                        : toast.error(
                            "Only Approved templates can be selected for sending.",
                          )
                    }
                  ...
                  >
                    {isSelected && (
                      <div className="absolute top-0 right-0 w-12 h-12 overflow-hidden pointer-events-none z-20">
                        <div className="absolute top-0 right-0 w-[200%] h-[200%] bg-emerald-500 origin-bottom-left rotate-45 translate-x-1/2 -translate-y-1/2 flex items-end justify-center pb-1">
                          <CheckCircle2
                            size={12}
                            className="text-white -rotate-45 mb-1"
                          />
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
                      <div className="flex-1 min-w-0 pr-6">
                        <h4 className="font-extrabold text-slate-800 text-[15px] leading-snug truncate">
                          {tpl.name}
                        </h4>
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          ...
                          <span className="text-[9px] font-bold text-slate-600 uppercase bg-slate-50 px-2 py-0.5 rounded border border-slate-200">
                            {tpl.language}
                          </span>
                        ...
                        <div className="bg-[#EFEAE2] rounded-xl p-3 h-full relative overflow-hidden flex flex-col min-h-[120px]">
                          ...
                          <div className="bg-white rounded-lg rounded-tl-none p-2.5 shadow-sm max-w-[90%] relative z-10 text-[12px] leading-relaxed text-slate-800 border border-slate-100">
                            ...
                            {tpl.headerType === "TEXT" && tpl.headerText && (
                              <div className="font-extrabold text-[13px] mb-1 leading-snug">
                                {tpl.headerText}
                              </div>
                            )}
                          ...
                          {tpl.buttons && tpl.buttons.length > 0 && (
                            <div className="mt-1 flex flex-col gap-1 max-w-[90%] relative z-10">
                              {tpl.buttons.map((btn, i) => (
                                <div
                                  key={i}
                                  className="bg-white rounded-lg shadow-sm border border-slate-100 py-1.5 px-2 flex items-center justify-center gap-1.5 text-[11px] font-bold text-[#00a884]"
                                >
                                  {btn.type === "URL" ? (
