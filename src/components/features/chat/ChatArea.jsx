...
const TemplateBubble = memo(function TemplateBubble({
  onSelect,
  onManage,
  onClose,
}) {
    ...
    <div className="absolute bottom-[70px] right-4 w-72 sm:w-80 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-200/60 flex flex-col overflow-hidden animate-in zoom-in-95 fade-in duration-200 z-50 origin-bottom-right">
      ...
      <div className="p-3 border-t border-slate-100 bg-slate-50">
        <button
          ...
          className="w-full py-2.5 bg-white border border-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-100 hover:text-slate-800 transition-colors shadow-sm flex items-center justify-center gap-2"
        >
...
const TemplateSidebar = memo(function TemplateSidebar({ open, onClose }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        name: nameInput.trim() || `Template ${templates.length + 1}`,
        sid,
      };
      ...
      if (editId) updateTemplate(editId, json.data);
      else addTemplate(json.data);
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed top-0 right-0 h-[100dvh] w-full sm:w-[420px] bg-white border-l border-slate-200 shadow-2xl z-[1000] flex flex-col transform transition-transform duration-300 ease-out ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="p-6 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
          ...
          <button
            className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-400 hover:text-rose-500 transition-all"
            onClick={onClose}
          >
            <X size={16} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-5 bg-slate-50 flex flex-col gap-3 custom-scrollbar">
          {loading && templates.length === 0 ? (
            ...
            templates.map((tpl) => (
              <div
                key={tpl._id || tpl.sid}
                className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3 transition-all hover:border-slate-300 shadow-sm animate-in fade-in slide-in-from-bottom-2"
              >
                <div className="w-9 h-9 rounded-lg shrink-0 flex items-center justify-center bg-slate-50 border border-slate-200 text-slate-400">
                  <Tag size={15} />
                </div>
                <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    className="w-7 h-7 rounded-md flex items-center justify-center text-slate-400 hover:bg-blue-100 hover:text-blue-600 transition-colors"
                    onClick={() => {
                      setEditId(tpl._id);
                  ...
                  <button
                    className="w-7 h-7 rounded-md flex items-center justify-center text-slate-400 hover:bg-rose-100 hover:text-rose-600 transition-colors"
        ...
        <div className="shrink-0 p-5 sm:p-6 bg-white border-t border-slate-200">
          {!showAddForm ? (
            <button
              className="w-full py-3 bg-emerald-50 border border-dashed border-emerald-200 rounded-xl text-[#00a884] text-sm font-bold flex items-center justify-center gap-2 hover:bg-emerald-100 transition-all"
            ...
            <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2">
              <div>
                ...
                <input
                  type="text"
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-[#00a884] focus:ring-2 focus:ring-[#00a884]/10"
                  value={nameInput}
              ...
              <div>
                ...
                <input
                  type="text"
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 font-mono text-sm text-slate-700 outline-none focus:border-[#00a884] focus:ring-2 focus:ring-[#00a884]/10"
                  value={sidInput}
              ...
              <div className="flex gap-2 mt-2">
                <button
                  className="flex-1 py-2.5 bg-slate-100 rounded-lg text-slate-600 text-sm font-bold hover:bg-slate-200 transition-all"
                  onClick={resetForm}
                >
                  Cancel
                </button>
                <button
                  className="flex-[2] py-2.5 bg-[#00a884] text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:bg-[#059669] transition-all disabled:opacity-50"
                  onClick={handleSave}
                  disabled={adding || !sidInput.trim()}
                >
                  {adding ? (
  ...
  return (
    <div className="relative bg-[#f0f2f5] p-3 px-4 border-t border-slate-200 flex items-end gap-3 z-20">
        ...
        <div className="absolute bottom-[70px] left-4 mb-2 w-[calc(100%-2rem)] max-w-sm bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-emerald-500/30 p-4 z-50 animate-in zoom-in-95">
          ...
          <div className="bg-slate-50 border border-slate-100 rounded-lg p-2.5 mb-4 max-h-24 overflow-y-auto custom-scrollbar">
            <p className="text-[11px] text-slate-600 font-mono leading-relaxed whitespace-pre-wrap">
              {varTemplate.template.body}
            </p>
          </div>

          <div className="space-y-3 mb-4 max-h-40 overflow-y-auto custom-scrollbar pr-1">
            {Array.from({ length: varTemplate.reqCount }, (_, i) => i + 1).map(
              (num) => (
            ...
            onClick={() => {
              for (let i = 1; i <= varTemplate.reqCount; i++) {
                if (!templateVars[i] || templateVars[i].trim() === "") {
                  return toast.error(
                    `Please enter a value for variable {{${i}}}`,
                  );
                }
              }
      ...
      <button
        ...
        className={`p-3 rounded-full shadow transition-all flex-shrink-0 mb-0.5 ${text.trim() ? "bg-[#00a884] text-white hover:bg-emerald-700" : "bg-slate-200 text-slate-400"}`}
      >
        {sending ? (
      ...
      <button
        ...
        className={`p-3 flex-shrink-0 rounded-full transition-all shadow-sm ${showBubble ? "bg-emerald-100 text-emerald-600" : "bg-white text-slate-500 hover:bg-slate-100"}`}
...
const ChatHeader = memo(function ChatHeader({
  ...
  const displayName =
    activeChat.name ||
    (activeChat.phone ? activeChat.phone.replace("whatsapp:", "") : "Unknown");
    ...
    <div className="bg-[#f0f2f5] border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-sm z-20">
      <div className="flex items-center gap-3">
        ...
        <div
          ...
          onClick={onInfo}
        >
          <div className="w-10 h-10 rounded-full bg-slate-300 flex items-center justify-center text-slate-600 font-bold shrink-0">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={onToggle}
          disabled={isToggling}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold transition-all shadow-sm ${isChatClosed ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100" : "bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200"}`}
        >
          {isToggling ? (
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 bg-blue-50 text-blue-800 hover:bg-blue-100 px-3 py-1.5 rounded-lg text-sm font-bold transition-colors shadow-sm"
            title="Change Status"
          >
            <FileText size={16} />
        ...
        <button
          onClick={onReminder}
          className="p-2 text-indigo-500 hover:bg-indigo-200 rounded-full transition-colors"
          disabled={true}
...
const MessageList = memo(function MessageList({
  ...
  const isNewHandler =
    activeChat?.lastClosedBy && activeChat.lastClosedBy !== userName;
      ...
      {isNewHandler && (
        <div className="flex justify-center mb-4 sticky top-0 z-10">
          <div className="bg-amber-50 text-amber-800 border border-amber-200 px-4 py-2 rounded-lg text-xs font-medium shadow-sm flex items-center gap-2">
            <Info size={14} /> This lead was previously handled by{" "}
            <strong>{activeChat.lastClosedBy}</strong>
          </div>
        </div>
      )}
    ...
    <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95">
      ...
      <div className="p-6 space-y-4">
        <textarea
          ...
          className="w-full p-4 text-sm bg-emerald-50 border border-emerald-100 rounded-2xl outline-none"
          rows={4}
          autoFocus
        />
        <button
...
const MediaViewer = ({ media, onClose }) => (
  ...
  >
    ...
    <div
      className="relative max-w-5xl max-h-full w-full flex justify-center"
      onClick={(e) => e.stopPropagation()}
    >
      {media.type?.includes("video") ? (
        ...
        <iframe
          src={media.url}
          className="w-[90vw] md:w-[80vw] h-[90vh] bg-white rounded shadow-2xl"
        />
      ) : (
        <img
          src={media.url}
