...
export default function TemplateManager() {
  ...
  if (!isAuthorized) return <div className="flex h-[100dvh] bg-slate-50 overflow-hidden relative"><Sidebar role={session?.user?.role} mobileOpen={mobileMenuOpen} setMobileOpen={setMobileMenuOpen} /><div className="flex flex-1 w-full h-full flex-col items-center justify-center p-6 text-center"><ShieldAlert size={80} className="text-rose-400 mb-6" /><h2 className="text-3xl font-extrabold text-slate-800">Clearance Required</h2></div></div>;
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative">
        <header className="h-auto md:h-20 px-6 py-4 md:py-0 bg-white border-b border-slate-200 flex items-center justify-between shrink-0 z-20 shadow-sm gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-extrabold text-slate-800 tracking-tight flex items-center gap-2"><LayoutTemplate className="text-[#00a884]" size={24} /> Template Builder</h1>
                ...
                <div className="relative w-full max-w-[360px] aspect-[9/19] bg-slate-900 rounded-[3rem] p-[10px] shadow-2xl border-[1px] border-slate-700/50 before:absolute before:inset-0 before:rounded-[3rem] before:shadow-[inset_0_0_2px_rgba(255,255,255,0.2)] mx-auto">
                  ...
                  <div className="w-full h-full bg-[#EFEAE2] rounded-[2.2rem] overflow-hidden flex flex-col relative z-20">
                    ...
                    <div className="flex-1 p-4 overflow-y-auto custom-scrollbar flex flex-col justify-end z-10 relative space-y-4">
                      ...
                      <div className="flex justify-center mb-2"><div className="bg-[#FFEECD] text-slate-600 text-[11.5px] font-medium px-4 py-2 rounded-xl shadow-sm text-center leading-relaxed max-w-[95%]"><AlertCircle size={14} className="inline-block mr-1.5 text-amber-600 -mt-0.5" />Messages and calls are end-to-end encrypted. No one outside of this chat, not even WhatsApp, can read or listen to them. Tap to learn more.</div></div>
            ...
            <div className="p-5 md:p-6 lg:px-8 border-b border-slate-100 bg-white flex flex-col md:flex-row md:items-center justify-between gap-5">
              <div>
                <h3 className="font-extrabold text-slate-900 flex items-center gap-2.5 text-[22px] tracking-tight">
                  <Database size={24} className="text-[#00a884]" /> Template Library
                </h3>
                      ...
                      {currentTemplates.map((tpl) => (
                          <div key={tpl._id} className="p-5 flex flex-col gap-4 hover:bg-slate-50/50 transition-colors">
                              ...
                              <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 flex items-center gap-2 text-slate-600 w-max">
                                  ...
                                  {tpl.templateType === "WHATSAPP_CARD" && <ImageIcon size={14} className="text-slate-400 shrink-0" />}
