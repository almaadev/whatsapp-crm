...
export default function TwilioEnterpriseDashboard() {
  ...
  const [clientUndeliveredOnly, setClientUndeliveredOnly] = useState(false);
  ...
  const handleDirectExport = async () => {
    ...
    try {
      ...
      if (apiStatusFilter !== "all")
        url += `&status=${encodeURIComponent(apiStatusFilter)}`;
      ...
      if (clientUndeliveredOnly) url += `&undeliveredOnly=true`;

      const res = await templateRepository.syncTwilioTemplates({
        responseType: "blob",
      });
  ...
  const fetchTwilioData = useCallback(async () => {
    ...
    try {
      ...
      if (startDate) params.startDate = startDate.split("T")[0];
      if (endDate) params.endDate = endDate.split("T")[0];
      if (apiStatusFilter && apiStatusFilter !== "all")
        params.status = apiStatusFilter;
      if (debouncedSearchQuery) params.search = debouncedSearchQuery;

      const { data } = await templateRepository.syncTwilioTemplates({ params });
  ...
  }, [status, isAuthorized, fetchTwilioData]);
  ...
  const filteredMessages = useMemo(() => {
    return twilioData.messages.filter((msg) => {
      if (searchQuery) {
      ...
      if (clientMediaOnly && (msg.numMedia || msg.mediaCount || 0) === 0)
        return false;
      if (clientFailedOnly && !["failed"].includes(msg.status?.toLowerCase()))
        return false;
      if (clientUndeliveredOnly && msg.status?.toLowerCase() !== "undelivered")
        return false;
      return true;
    });
  }, [
  ...
  const liveMetrics = useMemo(() => {
    ...
    return {
      ...
      totalInr: (totalUsd * exchangeRate).toFixed(2),
    };
  }, [filteredMessages, exchangeRate]);
    ...
    setTimeout(() => {
      if (type === "csv") {
        ...
        const csvRows = filteredMessages.map((msg) => {
          ...
          const inrCost = (
            Math.abs(Number(msg.price || 0)) * exchangeRate
          ).toFixed(4);
          return [
            msg.id,
            date,
  ...
  const handleSaveExchangeRate = async () => {
    if (!tempRate || isNaN(tempRate) || Number(tempRate) <= 0)
      return toast.error("Valid rate required");
    setSavingRate(true);
    try {
      const { data } = await templateRepository.syncTwilioRate({
        rate: tempRate,
      });
      if (data.success) {
  ...
  const currentBalanceNum = Number(twilioData.balance) || 0;
      ...
      <AnimatePresence>
        {exportLoading && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[9999] bg-slate-900/80 backdrop-blur-lg flex flex-col items-center justify-center text-white"
          >
            <Loader2 size={56} className="animate-spin text-emerald-400 mb-6" />
            <h2 className="text-2xl font-black tracking-widest uppercase text-emerald-50">
              Preparing Enterprise Export
            </h2>
            <p className="text-sm font-medium text-slate-300 mt-3 text-center max-w-sm">
              Generating optimized CSV stream from the server. <br /> Please do
            ...
            >
              <div className="p-6 pb-0">
                <div className="flex items-center gap-3 mb-4">
                  <div
                    className={`p-3 rounded-2xl ${isDangerousQuery ? "bg-rose-100 text-rose-600" : "bg-amber-100 text-amber-600"}`}
                  >
                ...
                <p className="text-sm text-slate-600 font-medium leading-relaxed">
                  You are attempting to fetch a{" "}
                  <strong className="text-slate-900">
                    Maximum Volume Archive
                  </strong>{" "}
                  of Twilio logs. This operation retrieves thousands of records
                  and consumes high API resources.
                </p>

                {isDangerousQuery && (
                  <div className="mt-4 p-4 bg-rose-50 border border-rose-200 rounded-xl">
                    <h4 className="text-sm font-bold text-rose-800 flex items-center gap-2 mb-1">
                      <ShieldAlert size={16} /> Warning: Unfiltered Query
                    </h4>
                    <p className="text-xs font-medium text-rose-700 leading-relaxed">
                      You have not applied any Date Range or Search filters.
                      Loading this directly into the UI may freeze your browser.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-6 pt-5 flex flex-col gap-3">
                <button
                  ...
                  className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-md"
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-sm transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="py-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-xl font-bold text-sm transition-colors"
                  >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">
                    Enterprise Communication Analytics & Billing
                  </p>
                </div>
              </div>
                ...
                <div className="relative z-10 flex flex-col gap-6">
                      ...
                      {isEditingRate ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="w-16 bg-white/20 border border-white/30 text-white text-xs px-2 py-1 rounded outline-none"
                            autoFocus
                          />
                          <button
                        ...
                        >
                          ...
                          <Edit2
                            size={12}
                            className="text-slate-400 group-hover/edit:text-white transition"
                          />
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-baseline gap-1.5 opacity-90">
                      ...
                      <span className="text-[10px] font-bold text-emerald-200/70 uppercase">
                        INR
                      </span>
                    </div>
                  </div>
                  {currentBalanceNum < 5 && (
                    <div className="flex items-center gap-3 bg-rose-500/20 border border-rose-500/30 text-rose-100 px-4 py-3 rounded-xl backdrop-blur-md">
                      ...
                      <p className="text-[11px] sm:text-xs font-bold">
                        Low Balance Warning. Please recharge.
                      </p>
                    </div>
                  )}
                </div>
                ...
                {[
                  {
                    ...
                    val: liveMetrics.total || 0,
                    color: "slate",
                  },
                  {
                    label: "Filtered Delivered",
                    val: liveMetrics.delivered || 0,
                    color: "emerald",
                  },
                  {
                    label: "Filtered Failed",
                    val: liveMetrics.failed || 0,
                    color: "rose",
                  },
                  {
                    label: "Rich Media",
                    val: liveMetrics.media || 0,
                    color: "amber",
                  },
                  {
                    label: "Outbound",
                    val: liveMetrics.outbound || 0,
                    color: "blue",
                  },
                  {
                    label: "Inbound",
                    val: liveMetrics.inbound || 0,
                    color: "indigo",
                  },
                  {
                    label: "Filtered Cost (USD)",
                    val: `$${liveMetrics.totalUsd}`,
                    color: "slate",
                  },
                  {
                    label: "Filtered Cost (INR)",
                    val: `₹${liveMetrics.totalInr}`,
                    color: "emerald",
                  },
                ].map((stat, i) => (
                  <motion.div
                    ...
                    key={i}
                    className={`bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm relative overflow-hidden group flex flex-col justify-between`}
                  >
                    <div
                    ...
                    <p
                      className={`text-xl sm:text-2xl font-black text-${stat.color}-600 relative z-10 truncate`}
                    >
                ...
                <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full lg:w-auto shrink-0">
                  <div className="flex bg-white border border-slate-200 rounded-lg overflow-hidden flex-1 sm:flex-none">
                    <input
                      ...
                      className="px-2 sm:px-3 py-2 text-[11px] sm:text-xs font-bold text-slate-700 outline-none w-full sm:w-[150px] bg-transparent"
                      title="Start Date"
                    />
                ...
                <div className="flex items-center gap-2 w-full lg:w-auto mt-1 lg:mt-0 shrink-0 border-t lg:border-t-0 border-slate-200 pt-2 lg:pt-0">
                  <div className="relative flex-1 lg:flex-none">
                    <select
                      ...
                      className="w-full bg-white border border-slate-200 rounded-lg px-2 py-2 text-[11px] sm:text-xs font-bold text-slate-700 outline-none cursor-pointer  "
                    >
                  ...
                  <AnimatePresence>
                    {showExportMenu && (
                      <motion.div
                        ...
                        className="absolute right-0 mt-2 w-36 bg-white border border-slate-200 shadow-xl rounded-xl overflow-hidden py-1 z-50"
                      >
                        <button
                  ...
                  <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row flex-wrap gap-4 md:gap-6 md:items-center">
                      ...
                      <div className="flex bg-slate-100 p-1 rounded-lg w-full md:w-auto">
                        {["all", "inbound", "outbound"].map((dir) => (
                          <button
                            key={dir}
                            onClick={() => {
                              setClientDirection(dir);
                              setCurrentPage(1);
                            }}
                            className={`flex-1 md:flex-none px-3 sm:px-4 py-1.5 rounded-md text-[11px] sm:text-xs font-bold capitalize transition-all ${clientDirection === dir ? "bg-white shadow-sm text-emerald-600" : "text-slate-500 hover:text-slate-700"}`}
                          >
                            {dir}
                          </button>
                        ))}
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full md:w-auto mt-2 md:mt-0">
                      <label className="flex items-center gap-2 cursor-pointer group bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-lg sm:rounded-none border sm:border-0 border-slate-200">
                        <input
                          type="checkbox"
                          checked={clientMediaOnly}
                          onChange={(e) => {
                            setClientMediaOnly(e.target.checked);
                            setCurrentPage(1);
                          }}
                      ...
                      <label className="flex items-center gap-2 cursor-pointer group bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-lg sm:rounded-none border sm:border-0 border-slate-200">
                        <input
                          type="checkbox"
                          checked={clientFailedOnly}
                          onChange={(e) => {
                            setClientFailedOnly(e.target.checked);
                            setCurrentPage(1);
                          }}
                      ...
                      <label className="flex items-center gap-2 cursor-pointer group bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-lg sm:rounded-none border sm:border-0 border-slate-200">
                        <input
                          type="checkbox"
                          checked={clientUndeliveredOnly}
                          onChange={(e) => {
                            setClientUndeliveredOnly(e.target.checked);
                            setCurrentPage(1);
                          }}
                          className="w-4 h-4 rounded text-rose-500 focus:ring-rose-500 border-slate-300"
                        />
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 overflow-hidden flex flex-col flex-1 min-h-[700px]">
              {loading || fetching ? (
                <div className="flex-1 flex flex-col items-center justify-center p-12 sm:p-20 min-h-[300px]">
                  ...
                  <p className="mt-4 text-xs sm:text-sm font-bold text-slate-400 uppercase tracking-widest animate-pulse">
                    Syncing Twilio Logs...
                  </p>
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 sm:p-20 text-center min-h-[300px]">
                  ...
                  <p className="text-xs sm:text-sm font-medium text-slate-500 mt-2 max-w-sm px-4">
                    We couldn't find any messages matching your current filter
                    criteria.
                  </p>
                  <button
                    onClick={() => {
                      ...
                      setClientDirection("all");
                      setClientMediaOnly(false);
                      setClientFailedOnly(false);
                      setActiveDateChip("all");
                      setStartDate("");
                      setEndDate("");
                    }}
                    className="mt-6 text-sm font-bold text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-4 py-2 rounded-lg transition-colors"
                  >
                    Clear All Filters
                  </button>
                </div>
                      ...
                      <tbody className="divide-y divide-slate-100 text-[13px] font-medium text-slate-700 relative">
                        <AnimatePresence initial={false}>
                          {paginatedMessages.map((msg) => {
                            ...
                            const previewWord = msg.body
                              ? msg.body.split(" ").slice(0, 4).join(" ") +
                                (msg.body.split(" ").length > 4 ? "..." : "")
                              : "Media Only";
                            const cost = Number(msg.costUsd || msg.price || 0);
                            const inrCost = cost * exchangeRate;

                            return (
                              <React.Fragment key={msg.id}>
                                <tr
                                  onClick={() => toggleExpand(msg.id)}
                                  className={`cursor-pointer transition-colors group ${isExpanded ? "bg-slate-50/80" : "hover:bg-slate-50/50"}`}
                                >
                                  <td className="px-6 py-4 pl-8 text-slate-600 font-semibold">
                                    {formatSafeDate(
                                      msg.timestamp || msg.dateSent,
                                    )}
                                  </td>
                                  <td className="px-6 py-4">
                                    <div className="flex items-center gap-2.5">
                                      <div
                                        className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 border ${isOutbound ? "bg-indigo-50 border-indigo-100 text-indigo-500" : "bg-emerald-50 border-emerald-100 text-emerald-500"}`}
                                      >
                                        {isOutbound ? (
                                          <Smartphone size={12} />
                                        ) : (
                                  ...
                                  <td className="px-6 py-4">
                                    <div className="flex items-center gap-2 text-slate-600 max-w-[250px]">
                                      ...
                                      <span className="truncate group-hover:text-emerald-600 transition-colors">
                                        {previewWord}
                                      </span>
                                    </div>
                                  </td>
                                          ...
                                          <div className="p-6 pl-8 flex gap-8">
                                            ...
                                            <div className="w-[300px] shrink-0 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                                              <div>
                                                ...
                                                <p className="font-mono text-[11px] text-slate-700 break-all bg-slate-50 p-2 rounded-lg border border-slate-100 selection:bg-emerald-200 select-all">
                                                  {msg.id}
                                                </p>
                                              </div>
                      ...
                      {paginatedMessages.map((msg) => {
                        ...
                        return (
                          <motion.div
                            layout
                            key={msg.id}
                            className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col w-full"
                          >
                            <div
                              onClick={() => toggleExpand(msg.id)}
                              className="p-4 flex flex-col gap-3 cursor-pointer select-none touch-manipulation w-full overflow-hidden"
                            >
                              <div className="flex justify-between items-start gap-2 w-full">
                                <div className="flex items-start gap-2.5 overflow-hidden flex-1 min-w-0">
                                  <div
                                    className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border mt-0.5 ${isOutbound ? "bg-indigo-50 border-indigo-100 text-indigo-500" : "bg-emerald-50 border-emerald-100 text-emerald-500"}`}
                                  >
                                    {isOutbound ? (
                                  ...
                                  <div className="flex flex-col overflow-hidden min-w-0">
                                    <p className="font-mono text-[13px] font-bold text-slate-800 truncate block">
                                      {isOutbound ? msg.to : msg.from}
                                    </p>
                              ...
                              <div
                                className={`text-[13px] text-slate-600 font-medium leading-relaxed bg-slate-50 p-3 rounded-xl border border-slate-100 break-words ${!isExpanded && "line-clamp-2"}`}
                              >
                                {hasMedia && !isExpanded && (
                                  <Paperclip
                                    size={12}
                                  ...
                                  <div className="p-4 space-y-3 bg-slate-50/50 w-full overflow-hidden">
                                    <div className="flex flex-col gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm w-full">
                                      ...
                                      <div className="w-full min-w-0">
                                        ...
                                        <p className="text-[10px] font-mono text-slate-500 break-all bg-slate-50 p-2 rounded-lg border border-slate-100 mt-1 select-all w-full leading-relaxed">
                                          {msg.id}
                                        </p>
                                      </div>
                                    </div>
                  ...
                  <div className="p-4 lg:p-5 border-t border-slate-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-4 mt-auto shrink-0 relative">
                    <div className="flex items-center justify-between w-full sm:w-auto gap-4">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <span className="text-[11px] sm:text-xs font-bold text-slate-500 hidden sm:block">
                          Rows:
                        </span>
                        <select
                          value={itemsPerPage}
