...
import {
  ...
  cleanPhoneNumber,
} from "@/shared/utils/customerResolver";
...
export default function ReportsPage() {
  ...
  const [associatePerformanceSummary, setAssociatePerformanceSummary] =
    useState([]);
  ...
  const [associatePagination, setAssociatePagination] = useState({
    total: 0,
    page: 1,
    limit: 25,
    pages: 1,
  ...
  useEffect(() => {
    if (user && !isSuperAdminUser && user.branch) {
      ...
      setAssociateReportFilters((prev) => ({
        ...prev,
  ...
  const filteredAssociatesDropdown = useMemo(() => {
    ...
    const activeFilters =
      activeTab === "customer" ? customerReportFilters : associateReportFilters;
    ...
    return allAssociatesList.filter((assoc) => {
      ...
      if (
        ...
        !includeMe &&
        loggedInUserId &&
        (assoc.id?.toString() === loggedInUserId ||
          assoc._id?.toString() === loggedInUserId)
      ) {
        return false;
      }
      ...
      if (activeFilters.branchId !== "all") {
        const matchedBranch = availableBranches.find(
          (b) => b._id === activeFilters.branchId,
        );
        const branchName = matchedBranch
          ? matchedBranch.name
          : activeFilters.branchId;
        if (
          assoc.branch !== branchName &&
          assoc.branch !== activeFilters.branchId
        )
          return false;
      }
      return true;
    ...
    try {
      ...
      if (payload.success) {
        ...
        setCustomerPagination(
          payload.pagination || { total: 0, page: 1, limit: 25, pages: 1 },
        );
      } else {
  ...
  const fetchAssociatePerformanceSummary = useCallback(async () => {
    ...
    try {
      ...
      const payload = await res.json();
      if (payload.success) {
        setAssociatePerformanceSummary(
          payload.associatePerformanceSummary || [],
        );
        ...
        toast.error(
          payload.error || "Failed to fetch associate performance summaries.",
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleResetFilters = () => {
    const defaultBranch = isSuperAdminUser
      ? "all"
      : user?.branch?.toString() || "all";
  ...
  const reportExportService = async (reportType, exportMode, formatType) => {
    try {
      ...
      let query = "";
      if (exportMode === "all") {
        const targetBranch = isSuperAdminUser
          ? "all"
          : user?.branch?.toString() || "all";
      ...
      let assocSuffix = "";
      if (associateId !== "all") {
        const selectedAssoc = allAssociatesList.find(
          (a) => a.id === associateId || a._id === associateId,
        );
  ...
  const setActiveFilters =
    activeTab === "customer"
      ? setCustomerReportFilters
      : setAssociateReportFilters;
            ...
            <div className="flex items-center gap-2">
              ...
              {isFilteredExport ? (
                <>
                  <button
                    onClick={() =>
                      reportExportService(activeTab, "filter", "csv")
                    }
                  ...
                  <button
                    onClick={() =>
                      reportExportService(activeTab, "filter", "excel")
                    }
                ...
                <>
                  ...
                  <button
                    onClick={() =>
                      reportExportService(activeTab, "all", "excel")
                    }
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-750 border border-slate-700 text-slate-200 font-black text-xs rounded-lg transition-all shadow-sm cursor-pointer"
                  >
                    Export All (Excel)
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3.5 pt-2 border-t border-slate-800">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (activeTab === "customer") setCustomerPage(1);
                  else setAssociatePage(1);
                }}
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="bg-slate-800 border border-slate-700 rounded-lg py-1.5 px-2.5 text-xs font-semibold text-slate-200 outline-none cursor-pointer disabled:bg-slate-850 disabled:text-slate-500 focus:border-[#00a884]"
              >
                {isSuperAdminUser && <option value="all">All Roles</option>}
                ...
                {isSuperAdminUser ? (
                  ...
                  <option value={user?.role?.toLowerCase()}>
              ...
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden select-none animate-in fade-in duration-150 flex flex-col">
                {customerReportRows.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <h4 className="font-bold text-slate-750 text-sm">
                      No customer records found
                    </h4>
                    <p className="text-[11px] text-slate-450 mt-0.5">
                      No customer lifecycle logs match your active filters or
                      search terms.
                    </p>
                  </div>
                ) : (
                      ...
                      <table className="w-full text-left border-collapse min-w-[2100px]">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-black text-slate-500 uppercase tracking-widest sticky top-0 bg-slate-50 z-10">
                            ...
                            <th
                              className="py-2.5 px-3 select-none cursor-pointer hover:bg-slate-100"
                              onClick={() => handleCustomerSort("customerName")}
                            >
                              Customer Name{" "}
                              {customerSortBy === "customerName"
                                ? customerSortOrder === "asc"
                                  ? "▲"
                                  : "▼"
                                : ""}
                            </th>
                            <th className="py-2.5 px-3 select-none">
                              Phone Number
                            </th>
                            <th
                              className="py-2.5 px-3 select-none cursor-pointer hover:bg-slate-100"
                            ...
                            <th
                              className="py-2.5 px-3 select-none cursor-pointer hover:bg-slate-100"
                              onClick={() => handleCustomerSort("branch")}
                            >
                              Assigned Branch{" "}
                              {customerSortBy === "branch"
                                ? customerSortOrder === "asc"
                                  ? "▲"
                                  : "▼"
                                : ""}
                            ...
                            <th
                              className="py-2.5 px-3 select-none cursor-pointer hover:bg-slate-100"
                              onClick={() => handleCustomerSort("associate")}
                            >
                              Current Lead Owner{" "}
                              {customerSortBy === "associate"
                                ? customerSortOrder === "asc"
                                  ? "▲"
                                  : "▼"
                                : ""}
                            </th>
                            <th className="py-2.5 px-3.5 select-none max-w-xs">
                              Latest Remark
                            </th>
                            <th className="py-2.5 px-3 select-none">
                              Last Follow Up Date
                            ...
                            <th className="py-2.5 px-3.5 text-center select-none w-20">
                              Actions
                            </th>
                          </tr>
                        </thead>
                            ...
                            return (
                              <Fragment key={row.id}>
                                <tr className="hover:bg-slate-50/60 transition-colors border-b border-slate-100">
                                  ...
                                  <td
                                    className="py-2.5 px-3 whitespace-nowrap text-slate-650 max-w-[200px] truncate"
                                    title={row.handledByAssociates}
                                  >
                                    {row.handledByAssociates}
                                  </td>
                                  <td className="py-2.5 px-3 whitespace-nowrap">
                                  ...
                                  <td className="py-2.5 px-3 font-bold text-[#00a884] whitespace-nowrap">
                                    {row.currentLeadOwner}
                                  </td>
                                  <td
                                    className="py-2.5 px-3.5 max-w-sm truncate text-slate-500"
                                    title={row.latestRemark}
                                  >
                                    {row.latestRemark}
                                  </td>
                                  <td className="py-2.5 px-3 text-slate-500 whitespace-nowrap">
                                ...
                                {isExpanded && (
                                  <tr className="bg-slate-50/40">
                                    <td
                                      colSpan="15"
                                      className="p-3 border-l-4 border-[#00a884] bg-slate-50/20"
                                    >
                                            ...
                                            {row.timeline.map((hist, tIdx) => (
                                              ...
                                              >
                                                ...
                                                <div className="flex items-center gap-2">
                                                  ...
                                                  <span className="text-[10px] text-slate-450 font-semibold ml-auto">
                                                    {hist.date}
                                                  </span>
                                                </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden select-none animate-in fade-in duration-150 flex flex-col">
                {associatePerformanceSummary.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <p className="text-[11px] text-slate-450 mt-0.5">
                      No associates match your branch or department/role
                      filters.
                    </p>
                  </div>
                ) : (
                      ...
                      <table className="w-full text-left border-collapse min-w-[1500px]">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-black text-slate-500 uppercase tracking-widest sticky top-0 bg-slate-50 z-10">
                            ...
                            <th
                              className="py-2.5 px-3 select-none cursor-pointer hover:bg-slate-100"
                              onClick={() =>
                                handleAssociateSort("associateName")
                              }
                            >
                              Associate Name{" "}
                              {associateSortBy === "associateName"
                                ? associateSortOrder === "asc"
                                  ? "▲"
                                  : "▼"
                                : ""}
                            </th>
                            ...
                            <th
                              className="py-2.5 px-3 text-center select-none cursor-pointer hover:bg-slate-100"
                              onClick={() =>
                                handleAssociateSort("customersHandled")
                              }
                            >
                              Customers Handled{" "}
                              {associateSortBy === "customersHandled"
                                ? associateSortOrder === "asc"
                                  ? "▲"
                                  : "▼"
                                : ""}
                            </th>
                            <th className="py-2.5 px-3 text-center select-none">
                              Follow Ups
                            </th>
                            <th className="py-2.5 px-3 text-center select-none">
                              Closed
                        ...
                        <tbody className="divide-y divide-slate-150 text-xs font-medium text-slate-700">
                          {associatePerformanceSummary.map((row) => {
                            const isExpanded =
                              expandedAssociateId === row.associateId;
                            return (
                              <Fragment key={row.associateId}>
                                <tr className="hover:bg-slate-50/60 transition-colors border-b border-slate-100">
                                  <td className="py-2.5 px-3.5 text-center">
                                    <button
                                      onClick={() =>
                                        setExpandedAssociateId(
                                          isExpanded ? null : row.associateId,
                                        )
                                      }
                                      className="p-0.5 rounded hover:bg-slate-200 text-slate-500 cursor-pointer"
                                    >
                                  ...
                                  <td className="py-2.5 px-3 uppercase text-[10px] text-slate-500 whitespace-nowrap">
                                    {row.role}
                                  </td>
                                  <td className="py-2.5 px-3 uppercase text-[10px] text-slate-500 whitespace-nowrap">
                                    {row.department}
                                  ...
                                  <td className="py-2.5 px-3 text-center font-mono font-bold text-slate-800 whitespace-nowrap">
                                    {row.customersHandled}
                                  </td>
                                  <td className="py-2.5 px-3 text-center font-mono text-slate-600 whitespace-nowrap">
                                    {row.followUps}
                                  </td>
                                  <td className="py-2.5 px-3 text-center font-mono text-emerald-600 whitespace-nowrap">
                                    {row.closed}
                                  </td>
                                  <td className="py-2.5 px-3 text-center font-mono text-slate-500 whitespace-nowrap">
                                    {row.notInterested}
                                  </td>
                                  <td className="py-2.5 px-3 text-center font-mono text-blue-600 whitespace-nowrap">
                                    {row.pending}
                                  </td>
                                  <td className="py-2.5 px-3 text-center font-mono text-emerald-600 whitespace-nowrap">
                                    {row.newLeadsClosed}
                                  </td>
                                  <td className="py-2.5 px-3 text-center font-mono text-[#00a884] whitespace-nowrap">
                                    {row.existingLeadsClosed}
                                  </td>
