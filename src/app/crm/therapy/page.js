...
import { HeartPulse, ShieldAlert, Menu, Send, User, ChevronLeft, Check, CheckCheck, Clock, X, MapPin, FileText, Search, Info, Lock, ToggleRight, ToggleLeft, Unlock } from "lucide-react";
...
const getDisplayMessage = (chat) => {
    ...
    return chat.message || "No therapy messages yet";
};
...
function TherapyContent() {
    ...
    const isAuthorized = isAdminAuthorized || accessModules.includes("Therapy");
    ...
    useEffect(() => {
        if (isAuthorized) {
            // Initial fetch handled by debouncedSearch effect above
            ...
            // --- Pass debouncedSearch to socket events to maintain filtered list ---
            socket.on("new_therapy_message", () => fetchChats(debouncedSearch, false));
    ...
    const getChatClosedStatus = () => {
        if (!selectedChat?.history || selectedChat.history.length === 0) return false;

        const lastMsg = selectedChat.history[selectedChat.history.length - 1];

        if (lastMsg?.isChatClosed === false) return true;   // CLOSED
        if (lastMsg?.isChatClosed === true) return false;   // ACTIVE

        const msgTime = new Date(lastMsg?.createdAt || lastMsg?.timestamp || 0).getTime();
        return msgTime > 0 && (Date.now() - msgTime) > 60 * 60 * 1000;
    };
    const isChatCurrentlyClosed = getChatClosedStatus();

    const handleToggleChatStatus = async (e) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        const lastIndex = selectedChat.history.length - 1;
        const lastMessage = selectedChat.history[lastIndex];

        const newClosedState = isChatCurrentlyClosed;
        ...
        try {
            const res = await fetch("/api/chats/status", {
                ...
                body: JSON.stringify({
                    ...
                    isChatClosed: newClosedState,
                    chatType: selectedChat?.type || "Therapy"
                })
            });
            ...
            toast.success(
                newClosedState
                    ? "Chat Marked as Closed"
                    : "Chat Reopened & Active"
            );
                ...
                <div className="flex flex-1 overflow-hidden relative">
                    <div className={`flex flex-col bg-white border-r h-full z-10 ${selectedChat ? 'hidden md:flex' : 'flex w-full'} md:w-[400px] lg:w-[450px] flex-shrink-0 transition-all`}>
                        <div className="bg-[#f0f2f5] px-4 py-3 flex items-center justify-between border-b border-slate-200 h-[60px] shrink-0">
                            <div className="flex items-center gap-3"><div className="w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center text-white shadow-sm shrink-0"><HeartPulse size={20} /></div><h2 className="font-bold text-[#111b21] text-[16px]">Therapy Leads</h2></div>
                        </div>
                        ...
                        <div className="relative z-10 h-full flex flex-col">
                            {selectedChat ? (
                                <>
                                    <div className="bg-white px-4 py-3 border-b border-slate-200 flex items-center justify-between shrink-0 shadow-sm z-10">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="w-10 h-10 bg-gradient-to-tr from-purple-500 to-fuchsia-400 rounded-full flex items-center justify-center text-white shrink-0 shadow-sm cursor-pointer" onClick={() => setIsInfoOpen(true)}><User size={20} className="opacity-90" /></div>
                                        ...
                                        <div className="flex items-center gap-2 sm:gap-3">
                                            {selectedChat?.history?.length > 0 && (
                                                <button onClick={handleToggleChatStatus} disabled={isToggling} className={`hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-bold transition-all shadow-sm ${isChatCurrentlyClosed ? 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'}`} title={isChatCurrentlyClosed ? "Mark chat as Active" : "Mark chat as Closed"}>
                                                    {isToggling ? <span className="w-3 h-3 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin"/> : (isChatCurrentlyClosed ? <ToggleRight size={16} /> : <ToggleLeft size={16} />)}
                                                    <span>{isChatCurrentlyClosed ? "Active" : "Closed"}</span>
                                                </button>
                                            )}
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center border-b-[6px] border-purple-500">
                                    ...
                                    <h2 className="text-3xl font-light text-slate-700 mb-4">Therapy Inbox</h2>
                    ...
                    {selectedChat && <CustomerInfoPanel isOpen={isInfoOpen} onClose={() => setIsInfoOpen(false)} leadCategory="therapy" activeChat={selectedChat} />}
                </div>
            </div>
        </div>
    );
