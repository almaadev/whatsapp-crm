...
import { Package, ShieldAlert, Menu, Send, User, ChevronLeft, Check, CheckCheck, Clock, X, MapPin, FileText, Search, Info, Lock } from "lucide-react";
...
function ProductLeadContent() {
  ...
  const isAuthorized = hasAccess("Product Lead");
    ...
    <div className="flex h-[100dvh] bg-gray-100 overflow-hidden relative">
      ...
      <div className="flex flex-1 w-full h-full relative overflow-hidden flex-col">
        ...
        <div className="flex flex-1 overflow-hidden relative">
            <div className={`flex flex-col bg-white border-r h-full z-10 ${selectedChat ? 'hidden md:flex' : 'flex w-full'} md:w-[400px] lg:w-[450px] flex-shrink-0 transition-all`}>
                <div className="bg-[#f0f2f5] px-4 py-3 flex items-center justify-between border-b border-slate-200 h-[60px] shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white shadow-sm shrink-0"><Package size={20}/></div>
                        <h2 className="font-bold text-[#111b21] text-[16px]">Product Leads</h2>
                    </div>
                </div>
                ...
                <div className="relative z-10 h-full flex flex-col">
                    {selectedChat ? (
                            ...
                            <div className="bg-[#f0f2f5] px-4 py-2.5 border-b border-slate-200 flex items-center justify-between shrink-0 shadow-sm h-[60px]">
                                ...
                                <div className="flex items-center gap-2 sm:gap-3">
                                    ...
                                    <div className="relative" ref={statusMenuRef}>
                                        <button onClick={() => setIsStatusMenuOpen(!isStatusMenuOpen)} className="flex items-center gap-1.5 bg-blue-100 text-blue-800 hover:bg-blue-200 px-3 py-1.5 rounded-lg text-sm font-bold transition-colors shadow-sm">
                                            <FileText size={16} />
                                    ...
                                    <button onClick={() => setIsInfoOpen(true)} className="flex items-center justify-center p-2 text-slate-500 hover:text-blue-700 hover:bg-blue-100 rounded-full transition-all" title="Customer Info"><Info size={22} /></button>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center border-b-[6px] border-blue-500">
                            ...
                            <h2 className="text-3xl font-light text-[#41525d] mb-4">Product Leads Inbox</h2>
            ...
            <CustomerInfoPanel isOpen={isInfoOpen} onClose={() => setIsInfoOpen(false)} leadCategory="product" activeChat={selectedChat} />
        </div>
      </div>
    </div>
  );
