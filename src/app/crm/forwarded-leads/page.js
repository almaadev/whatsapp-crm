...
export default function ForwardedLeadsPage() {
      ...
      try {
        ...
        if (!isAdmin) {
          uniqueLeads = uniqueLeads.filter(
            (lead) =>
              lead.forwardedBy === session.user.name ||
              lead.lastForwardedName === session.user.name,
          );
        }
  ...
  const filteredLeads = leads.filter(
    (l) =>
      ...
      (l.forwardedBy &&
        l.forwardedBy.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (l.lastForwardedName &&
        l.lastForwardedName.toLowerCase().includes(searchTerm.toLowerCase())),
  );
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                Track internal lead transfers.
              </p>
            </div>
            <div className="relative w-full md:w-96 group">
              <Search
                className="absolute left-3 top-3 text-slate-400 group-focus-within:text-emerald-600 transition-colors"
                size={18}
              />
              <input
                type="text"
                placeholder="Search leads..."
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white shadow-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none text-sm transition-all"
                value={searchTerm}
                      ...
                      >
                        ...
                        <td className="px-4 py-4 text-slate-500 text-xs font-medium whitespace-nowrap">
                          <div className="flex items-center gap-1.5">
                            <Calendar size={12} />
                            {lead.forwardDate
                              ? parseDate(lead.forwardDate).toLocaleDateString()
                              : "-"}
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 text-center">
                          {lead.isClosed === "TRUE" ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200 whitespace-nowrap">
                              COMPLETED
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-100 whitespace-nowrap">
                              PENDING
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-4 text-right">
                          <button
                            onClick={() => handleNavigateToChat(lead)}
                            className="inline-flex items-center justify-center w-8 h-8 md:w-auto md:h-auto md:px-3 md:py-1.5 rounded-lg text-xs font-bold text-slate-500 hover:text-emerald-600 bg-white hover:bg-emerald-50 border border-slate-200 hover:border-emerald-200 transition-all shadow-sm"
                          >
