                  ...
                  <tbody className="divide-y divide-slate-100 text-sm">
                    {loading ? (
                      ...
                      <tr>
                        ...
                        >
                          No associates found matching criteria.
                        </td>
                      </tr>
                    ) : (
                      filteredAssociates.map((associate) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="font-mono text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded w-max mb-1 flex items-center gap-1.5">
                              <Phone size={12} /> {associate.number || "N/A"}
                            </div>
                            {associate.preferredName && (
                              <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider ml-1">
                                AKA: {associate.preferredName}
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-center">
                            <span
                              className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider border ${associate.active ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200"}`}
                            >
                              {associate.active ? (
                              ...
                              {deleteConfirmId === associate.id ? (
                                <div className="flex items-center gap-1 bg-rose-50 border border-rose-200 rounded-lg p-1 animate-in fade-in zoom-in-95">
                                  <button
                                    ...
                                    className="px-2 py-0.5 bg-rose-500 text-white text-[10px] font-bold rounded hover:bg-rose-600 transition"
                                  >
                                    Confirm
                                  </button>
                                  <button
                                ...
                                <button
                                  onClick={() =>
                                    setDeleteConfirmId(associate.id)
                                  }
              ...
              <div className="md:hidden flex flex-col p-4 gap-4 bg-slate-50">
                {loading ? (
                  ...
                  filteredAssociates.map((associate) => (
                    <div
                      key={associate.id}
                      className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex flex-col gap-3"
                    >
                      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center font-bold text-emerald-700 shrink-0 border border-emerald-100">
                            {associate.name.charAt(0).toUpperCase()}
                          </div>
                        ...
                        <span
                          className={`w-2 h-2 rounded-full shrink-0 ${associate.active ? "bg-emerald-500" : "bg-rose-500"}`}
                        ></span>
                      </div>
              ...
              <div className="px-6 py-5 flex items-center justify-between border-b border-slate-100 shrink-0">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium mt-1">
                    Add a new team member to the CRM platform.
                  </p>
                </div>
                <button
