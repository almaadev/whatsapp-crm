...
export async function GET(req) {
  try {
    ...
    if (searchQuery && searchQuery.trim() !== '') {
        ...
        const matchedMessagePhones = await TherapyMessage.distinct("phone", { phone: searchRegex });
        phones = [...new Set([...matchedCustomerPhones, ...matchedMessagePhones])];

        if (!phones.length) return NextResponse.json([]);
    } else {
        phones = await TherapyMessage.distinct("phone");
        if (!phones.length) return NextResponse.json([]);
    }
...
export async function POST(req) {
  ...
  try {
    ...
    const messageRecord = await Message.create({
      phone: formattedTo, message: `${message}\nFrom Therapy chat by ${session?.user?.name || session?.user?.email}`, direction: "OUTBOUND", status: twilioStatus, twilioSid, timestamp: new Date(),
    });

    if (global.io) {
      global.io.emit("new_therapy_message", { phone: formattedTo, message, direction: "OUTBOUND", timestamp: saved.timestamp ?? saved.createdAt ?? new Date() });
