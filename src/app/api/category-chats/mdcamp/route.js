...
export async function GET() {
    try {
        await connectDB();
        const targetType = "MD Camp";
        
        const specificPhones = await MDCampMessage.distinct('phone');
        ...
        const chats = leads.map(lead => {
            ...
            return {
                ...
                history: leadMsgs,
                message: leadMsgs.length === 0 ? "No MD Camp messages yet" : null // Alternate display text
            };
        }).sort((a, b) => {
...
export async function POST(req) {
    try {
        ...
        const newMessage = await MDCampMessage.create({
        ...
        if (global.io) {
            global.io.emit("new_mdcamp_message", { phone: formattedTo, message, direction: "OUTBOUND", timestamp: new Date().toISOString() });
        }
...
export async function GET() {
  try {
    ...
    const phones = await MDCampMessage.distinct("phone");
    ...
    const allMessages = await MDCampMessage
...
// ─────────────────────────────────────────────────────────────────────────────
//  POST /api/messages
//  Sends a WhatsApp message via Twilio and stores it in MDCampMessage only.
...
export async function POST(req) {
  try {
    ...
    // ── 4. Persist to MDCampMessage only ────────────────────────────────────
    ...
    if (global.io) {
      global.io.emit("new_MD_Camp_message", {
        phone:     formattedTo,
        message,
        direction: "OUTBOUND",
