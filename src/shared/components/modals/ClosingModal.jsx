...
const ClosingModal = ({ note, setNote, onClose, onSubmit }) => (
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm transition-opacity">
    <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95">
      ...
      <div className="p-6 space-y-4">
        <textarea value={note} onChange={(e) => setNote(e.target.value)} className="w-full p-4 text-sm bg-emerald-50 border border-emerald-100 rounded-xl outline-none" rows={4}  autoFocus />
